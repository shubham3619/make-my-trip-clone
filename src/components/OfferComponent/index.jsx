      ...
      <CardContent className="p-4 md:p-6">
        ...
        <div className="flex flex-wrap justify-between items-center mb-6">
          <div className="flex flex-wrap items-center gap-4 md:gap-6">
            <h2 className="text-2xl md:text-4xl font-extrabold text-gray-800">Offers</h2>
          ...
          <div className="flex items-center gap-2 mt-4 md:mt-0">
            ...
            <div className="flex gap-2">
              <Button
                ...
                onClick={handlePrev}
                disabled={isAtStart}
                className={`h-8 w-8 rounded-full ${isAtStart ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100'}`}
              >
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <Button
                ...
                onClick={handleNext}
                disabled={isAtEnd}
                className={`h-8 w-8 rounded-full ${isAtEnd ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100'}`}
              >
                <ChevronRight className="h-4 w-4" />
              </Button>
            </div>
        ...
        <div className="relative overflow-hidden">
          ...
          >
            {Array.from({ length: totalSlides }).map((_, groupIndex) => (
              <div
                key={groupIndex}
                className="w-full md:w-1/2 flex-shrink-0 grid grid-rows-1 md:grid-rows-2 gap-4"
              >
                {offers.slice(groupIndex * 2, groupIndex * 2 + 2).map((offer, index) => (
                  <Card key={index} className="bg-white shadow-sm hover:shadow-md transition-shadow">
                    <CardContent className="p-4">
                      <div className="flex flex-col md:flex-row gap-4">
                        ...
                        <div className="w-full md:w-2/3">
                          ...
                          <h3 className="text-md md:text-lg font-bold mb-1">{offer.title}</h3>
                          <p className="text-sm text-gray-600 mb-2">{offer.description}</p>
                          <Button
                            variant="link"
                            className="text-blue-500 float-right font-extrabold text-sm md:text-md hover:text-blue-600 p-0"
                          >
                            BOOK NOW
                          </Button>
                        </div>
